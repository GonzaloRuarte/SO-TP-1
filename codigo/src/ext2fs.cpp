...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number){
	//TODO: Ejercicio 2
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int blockGroupNumber = blockgroup_for_inode(inode_number);
	Ext2FSBlockGroupDescriptor* groupDescriptor = block_group(blockGroupNumber);
	unsigned int inodeIndexInTable = blockgroup_inode_index(inode_number);
	unsigned int inodesInblock = block_size/sizeof(Ext2FSInode);
	div_t inodeBlockIndex = div(inodeIndexInTable,inodesInblock);
	char buffer[block_size];
	read_block(groupDescriptor->inode_table + inodeBlockIndex.quot, buffer); 
	return &((Ext2FSInode)buffer)[inodeBlockIndex.rem];
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int first_indirection = block_size / sizeof(unsigned int);
	char buffer[block_size];

	if(block_number<12){
		return (inode->block)[block_number];
	}
	if(block_number<first_indirection+12){
		read_block((inode->block)[12], buffer);
		return ((unsigned int*)buffer)[block_number-12];
	}else{
		unsigned int index = block_number-first_indirection-12;
		div_t secondIndirection = div(index,first_indirection);
		read_block((inode->block)[13], buffer);
		read_block(((unsigned int*)buffer)[secondIndirection.quot],buffer);
		return ((unsigned int*)buffer)[secondIndirection.rem];
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	for(int i=0; i<12+first_indirection+(first_indirection*first_indirection);++i){

		unsigned int blockAddress = get_block_address(from,i);
		read_block(blockAddress,buffer);
		unsigned int j = 0;
		while (j  < block_size) {
			Ext2FSDirEntry* dir= &(Ext2FSDirEntry*)(buffer[j]);
			if(strcmp((char*)dir->name , filename)==0){
